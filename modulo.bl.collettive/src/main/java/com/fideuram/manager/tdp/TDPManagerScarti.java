 ...
 * Time: 12.58
 */
public class TDPManagerScarti {
    public static byte[] getPdfScartiTDP(File file) throws StampaCollettiveException, Exception {
        TdpDao dao = new TdpDao();
        try {
            Collettive collettive = dao.getScartoTDP (file);
            byte[] template=TemplateFactory.getTemplateFromRichiesta(ProcessiCollettive.SCARTI_TITOLI_DI_PERFEZIONAMENTO);
