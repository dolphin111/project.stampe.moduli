 ...
 * Time: 12.32
 */
public class FvInsiemePerNoi extends FVInsieme{
    public FvInsiemePerNoi(String dir) {
        super(dir);
    }
    ...
    public File generateSupplemento(PubblicazioneConsob doc) throws ProspettoDiOffertaException {
        //doc.setNotaIvass();
        doc.setNomeProdotto("Fideuram Vita Insieme Per Noi"); //TODO VERIFICARE SE GIà INSERITO
        doc.getSupplemento().setArt(Frasi.getFrase("fv.insieme.x.noi.art"));
        doc.getSupplemento().setPremessaParte1A(Frasi.getFrase("fv.insieme.x.noi.1.A"));
        doc.getSupplemento().setAggiornamento(Frasi.getFrase("fv.insieme.x.noi.aggiornamento"));
