...
import com.fideuram.processo.prospetti.FVInsiemeSintonia;
 ...
 *
 */
public class ControllerOffertaFVISintonia extends ControllerOffertaBase {
	
	List<String> anni = new ArrayList<String>();
	private List<String> na = Arrays.asList("n.a.", "na", "n.a", "na.", "N.A.", "NA", "N.A", "NA.", "NA");
	private List<String> nd = Arrays.asList("n.d.", "nd", "n.d", "nd.", "N.D.", "ND", "N.D", "ND.", "ND");
    ...
    public List<byte[]> avviaProcesso() throws ProspettoDiOffertaException{
    	PubblicazioneConsob p = loadDati(66);
    	p = setDatiGrafico(p);
    	p.setNomeProdotto("Fideuram Vita Sintonia");
    	p.setGemini(false);
    	GeneratoreBase generatoreFVInsiemeSintonia= new FVInsiemeSintonia(dir);
        List<byte[]> pdfs=new ArrayList<byte[]>();
        byte[] sintesi = stampaParteX(p, 7, generatoreFVInsiemeSintonia);
        sintesi = addPiedino(sintesi,"FVSintonia-NotaInformativa.pdf", true);
        pdfs.add(sintesi);

        sendPDFListToPuc(pdfs, 66);
    ...
    public List<byte[]> avviaProcessoSintesi() throws ProspettoDiOffertaException{
        ...
        GeneratoreBase generatoreFVInsiemeSintonia= new FVInsiemeSintonia(dir);
        List<byte[]> pdfs=new ArrayList<byte[]>();
        byte[] sintesi = stampaParteX(p, 6, generatoreFVInsiemeSintonia);
        sintesi = addPiedino(sintesi,"FVSintonia-SchedaSintetica.pdf", true);
        pdfs.add(sintesi);
    ...
    public List<byte[]> avviaProcessoRendiconto() throws ProspettoDiOffertaException{
        PubblicazioneConsob p = loadDati(661);
        p = setFlagPrimoFondoEsterno(p);
    	p = setDatiGrafico(p);
        p.setNomeProdotto("Fideuram Vita Sintonia");
        ...
        GeneratoreBase generatoreFVInsiemeSintonia= new FVInsiemeSintonia(dir);
        List<byte[]> pdfs=new ArrayList<byte[]>();
        byte[] rendiconto = stampaParteX(p, 5, generatoreFVInsiemeSintonia);
        rendiconto = addPiedino(rendiconto,"FVSintonia-Rendiconto.pdf", true);
        pdfs.add(rendiconto);
    ...
    public List<byte[]> avviaProcessoAddendum() throws ProspettoDiOffertaException{
        PubblicazioneConsob p = loadDati(662);
        p = setFlagPrimoFondoEsterno(p);
        p = setDatiGrafico(p);
        p.setNomeProdotto("Fideuram Vita Sintonia");
        p.setGemini(false);
        GeneratoreBase generatoreFVInsiemeSintonia= new FVInsiemeSintonia(dir);
        List<byte[]> pdfs=new ArrayList<byte[]>();
        byte[] sintesi = stampaParteX(p, 8, generatoreFVInsiemeSintonia);
        sintesi = addPiedino(sintesi,"FVSintonia-Addendum.pdf", false);
        pdfs.add(sintesi);
    			...
    			if(fund.getIsins() != null){
    				...
    				for(Isin isin : isins){
    					...
    					for(Ter t : ters){
    						...
    						if(t.getSpeseRevisioneCertificazionePatrimonio()!= null && !"---".equals(t.getSpeseRevisioneCertificazionePatrimonio())&&  !"".equals(t.getSpeseRevisioneCertificazionePatrimonio())&&  !nd.contains(t.getSpeseRevisioneCertificazionePatrimonio())){
    						...
    						if(t.getAltriOneri()!= null && !"---".equals(t.getAltriOneri())&&  !"".equals(t.getAltriOneri())&&  !nd.contains(t.getAltriOneri())){
    					...
    					if(barraGraph != null && !isin.isFondoInterno()){
	    					...
	    					System.out.println("LISTA BARRE CODICE IMPRESA: " + isin.getCodiceImpresa());
	...
	private Isin setMessaggiFondo(Isin isin) {
		...
		if(!na.contains(strumentiDerivati)){
